...
def novo_flashcard(request):
    ...
    if request.method == "GET":
            ...
            if categoria_filtrar:
                  flashcards = flashcards.filter(categoria_id = categoria_filtrar)
...
def relatorio(request, id):
      ...
      erros = desafio.flashcards.filter(acertou=False).filter(respondido=True).count()
      ...
      acertos_categoria = []
      for categoria in categorias:
            acertos_categoria.append(desafio.flashcards.filter(flashcard__categoria=categoria).filter(acertou=True).count())
      return render(request, 'relatorio.html', {'desafio': desafio, 'dados':dados, 'categorias':name_categoria,
